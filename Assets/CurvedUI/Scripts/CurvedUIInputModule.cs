...
#region EVENT PROCESSING - NEW UNITY INPUT SYSTEM w/ UNITY_XR -----------------------------------//
    ...
    protected override MouseState GetMousePointerEventData(int id)
    {
        ...
        currentPointedAt = ret.GetButtonState(PointerEventData.InputButton.Left)
    ...
    public void ScanForOculusPrefabs()
    {
        ...
        if (oculusCameraRig == null)
        {
            ...
            if (oculusCameraRig == null)  oculusCameraRig = FindObjectOfType<OVRCameraRig>();
        ...
        {
            if (FindObjectOfType<Oculus.Interaction.Input.HandSkeletonOVR>() is var skeleton 
                && skeleton != null 
    ...
    protected virtual void ProcessOculusVRController()
    {
        ...
        _isUsingOVRHandTracking = _activeOVRController == OVRInput.Controller.LHand 
                                     || _activeOVRController == OVRInput.Controller.RHand 
        ...
        if (_isUsingOVRHandTracking)
        {
            ...
            if (_ovrHandPointerPose == null)
            {
                ...
                _ovrHandPointerPose.InjectHand(UsedHand == Hand.Both || UsedHand == Hand.Right ? _ovrRightHand : _ovrLeftHand); 
                _ovrHandPointerPose.transform.SetParent(oculusCameraRig.transform);
            }
            
            //buttons state and pointer pose
            if ((UsedHand == Hand.Both || UsedHand == Hand.Right) && _ovrRightHand != null && _ovrRightHand.IsTrackedDataValid)
            {
                _ovrHandPointerPose.InjectHand(_ovrRightHand); 
                CustomControllerButtonState = _ovrRightHand.GetIndexFingerIsPinching();
            }
            else if ((UsedHand == Hand.Both || UsedHand == Hand.Left) && _ovrLeftHand != null && _ovrLeftHand.IsTrackedDataValid)
            {
                _ovrHandPointerPose.InjectHand(_ovrLeftHand); 
                CustomControllerButtonState = _ovrLeftHand.GetIndexFingerIsPinching();
            }
            else CustomControllerButtonState = false;
        ...
        if(!_isUsingOVRHandTracking) // Oculus Controllers --------------------------------------------------//
        {
            ...
            switch (_activeOVRController)
            {
                ...
                //edge cases
                default: CustomControllerRay = new Ray(ControllerTransform.position, ControllerTransform.forward); break;
            }
            ...
            var touchControllersUsed = _activeOVRController == OVRInput.Controller.Touch 
                                       || _activeOVRController == OVRInput.Controller.LTouch 
                                       || _activeOVRController == OVRInput.Controller.RTouch;
        
            if (usedHand == Hand.Both || !touchControllersUsed) //button is pressed on any controller
            {
    ...
    public bool IsUsingOvrHandTracking => _isUsingOVRHandTracking;
