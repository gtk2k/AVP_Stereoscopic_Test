...
#pragma warning restore 0649
        ...
        void Update()
        {
            ...
            //initialization
            if (!initialized && CurvedUIInputModule.Instance.ControllerTransform != null)
            {
            ...
            else if (LaserBeam.transform.parent != CurvedUIInputModule.Instance.ControllerTransform)
            {
                SwitchHandTo(CurvedUIInputModule.Instance.UsedHand);
            }
            ...
            if(autoSwitchHands){
                ...
                if (CurvedUIInputModule.Instance.IsUsingOvrHandTracking 
                         && CurvedUIInputModule.Instance.IsOculusHandPinching(CurvedUIInputModule.Hand.Left) 
                         && CurvedUIInputModule.Instance.UsedHand != CurvedUIInputModule.Hand.Left)
                {
                    SwitchHandTo(CurvedUIInputModule.Hand.Left);
                }
                else if (CurvedUIInputModule.Instance.IsUsingOvrHandTracking 
                         && CurvedUIInputModule.Instance.IsOculusHandPinching (CurvedUIInputModule.Hand.Right) 
                         && CurvedUIInputModule.Instance.UsedHand != CurvedUIInputModule.Hand.Right)
                {
                    SwitchHandTo(CurvedUIInputModule.Hand.Right);
                }
                //For Oculus Rift / Quest, we wait for the click before we change the pointer to the other hand
                else  if (IsButtonDownOnController(OVRInput.Controller.LTouch) 
                    && CurvedUIInputModule.Instance.UsedHand != CurvedUIInputModule.Hand.Left)
                {
                   SwitchHandTo(CurvedUIInputModule.Hand.Left);
                }
                else if (IsButtonDownOnController(OVRInput.Controller.RTouch) 
                         && CurvedUIInputModule.Instance.UsedHand != CurvedUIInputModule.Hand.Right)
                {
                   SwitchHandTo(CurvedUIInputModule.Hand.Right);
                }
        ...
        {
            return OVRInput.GetDown(CurvedUIInputModule.Instance.OculusTouchInteractionButton, cont) 
                   || (cont2 != OVRInput.Controller.None && OVRInput.GetDown(CurvedUIInputModule.Instance.OculusTouchInteractionButton, cont2));
        }
